...
class Record(QDialog):
    ...
    def insertOrUpdate(self,Id, Name, UID, Skills):
        db = pymysql.connect("localhost", "root", "root", "rtfr")
        cursor = db.cursor()
        sql = "SELECT * FROM employee WHERE ID=" + str(Id)
        cursor.execute(sql)
        isRecordExist = 0
        for row in cursor:
            print(row)
            isRecordExist = 1
        if isRecordExist == 1:
            sql = "UPDATE employee SET Name = '" + str(Name) + "', UID = '" + str(UID) + "', Skills = '" + str(Skills) + "' WHERE ID=" + str(Id)
        else:
            sql = "INSERT INTO employee(ID, Name, UID, Skills) Values(" + str(Id) + ",'" + str(Name) + "','" + str(UID) + "','" + str(Skills) + "')"
        cursor.execute(sql)
        db.commit()
        db.close()
